...
if debug:
    test_df = pd.read_csv(inpath+"train.csv", parse_dates=['click_time'], skiprows=range(1,nchunk), nrows=1000, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
else:
    test_df = pd.read_csv(inpath+"train.csv", parse_dates=['click_time'], skiprows=range(1,nrows-40000000), nrows=40000000, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
...
ra_val = RocAucEvaluation(validation_data=(val_df, y_val), interval = 1)
optimizer_adam = Adam(lr=0.001, decay=lr_decay)
model.compile(loss='binary_crossentropy',optimizer=optimizer_adam,metrics=['accuracy'])

model.summary()
class_weight = {0:.01,1:.99} # magic
model.fit(train_df, y_train, batch_size=batch_size, class_weight=class_weight, epochs=1, shuffle=True, validation_data = [val_df, y_val],verbose = 1, callbacks = [ra_val])
del train_df, y_train, val_df, y_val; gc.collect()
model.save_weights('dl_support.h5')

sub = pd.DataFrame()
test_df = pd.read_pickle('test.pkl.gz')
sub['app'] = test_df['app'].astype('int')
...
print("writing....")
sub.to_csv('DL_OOF.csv',index=False)